...
public class FileDestination: BaseDestination {
    ...
    /// before each write check on file size
    public var fileDestinationController: IFileDestinationController?
    ...
    func getLogFileURL() -> URL? {
        ...
        let newUrl = fileDestinationController?.createNewLogFileIfNeeded(currentURL: url) ?? url
        
        if newUrl != url {
            print("🤎", #function, "old url", url)
            print("🤎", #function, "new url", newUrl)
        }
    ...
    func saveToFile(str: String) -> Bool {
        guard let url = getLogFileURL() else { return false }
